...
-(void)updpostatus: (NSString *)xtype2 andpartail:(NSString *)partail{
    ...
    if (netStatus ==NotReachable) {
        ...
        [service xUpdateUserPurchaseOrder:self action:@selector(doapprovea:) xemail:[userInfo getUserName]  xpassword:[userInfo getUserPwd] xidcia:[[NSNumber numberWithInt:[userInfo getCiaId]] stringValue] xpoid:idpo1 xtype:xtype2 update:@"" vendorid:@"" delivery:pd.Delivery xlgsel:partail xcode:xcode EquipmentType:@"3" continueyn:@"0"];
        
        NSLog(@"%@\n%@\n%@\n%@\n%@\n%@\n%@\n", [userInfo getUserName], [userInfo getUserPwd], [[NSNumber numberWithInt:[userInfo getCiaId]] stringValue], idpo1, xtype2, pd.Delivery, xcode);
        NSLog(@"fasdfasdf");
    }
}
